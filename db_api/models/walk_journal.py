...
from bjb_toolkit.analytics.journal_analytics import get_walk_analytics
...
class WalkRecord:
    ...
    EXCEL_COLUMN_NAMES = ['Длительность, мин', 'Дата и время прогулки']
    LOGO_URL = 'http://i.piccy.info/i9/ee2b47e22f7c89e647a50f98d3ef86fd/1616183140/3173/1421782/Progulka.png'
    ...
    async def save(self):
        records = await bjb_api.journal.WalkRecord(self.token).create(
    ...
    async def delete(cls, token, record_id):
        await bjb_api.journal.WalkRecord(token).delete(record_id)

    @classmethod
    async def get_list(cls, token, day_from, day_to, offset=None, limit=None):
        if not offset and not limit:
            records, _ = await bjb_api.journal.WalkRecord(token).get_list(
                dt_after=day_from,
                dt_before=day_to,
            )
        else:
        ...
        return [WalkRecord(r['duration'], parse_json_dt(r['dt'])) for r in records]
    ...
    async def get_analytics(cls, records, timezone, name_btn='период'):
        fig, caption = get_walk_analytics(records, timezone, period_name=name_btn)
        return caption, fig
